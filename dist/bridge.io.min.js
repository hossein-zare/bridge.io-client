...
const{event=null,data=null}=json;if(event==='ack'&&this.isObject(data)&&data.ack&&Array.isArray(data.args)){if(data.ack in this.acknowledgments){this.acknowledgments[data.ack](...data.args);delete this.acknowledgments[data.acck]}}else if(event in this.events){this.events[event](data)}}})}
connect(){if(this.connection){if(this.connection.readyState===1)
return;this.dontFireCloseEvent=!0;this.disconnect()}
