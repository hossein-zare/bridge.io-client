...
class BridgeIO{constructor(server,config={}){this.server=server;this.config=Object.assign({protocol:[],attempts:null,timeout:2000},config);this.connection=null;this.opened=!1;this.attempts=0;this.successfulAttempts=0;this.disconnected=!1;this.dontFireCloseEvent=!1;this.casterId=400;this.acknowledgments={};this.events={open:()=>{},connection_ready:()=>{},close:()=>{},reconnecting:()=>{},reconnected:()=>{},}}
...
this.attempts=0;this.successfulAttempts++;this.heartbeat()});this.connection.addEventListener('close',(e)=>{clearTimeout(this.connection.pingTimeout);if(!this.dontFireCloseEvent)
...
const{event=null,data=null}=json;if(event==='ack'&&this.isObject(data)&&data.ack&&Array.isArray(data.args)){if(data.ack in this.acknowledgments){this.acknowledgments[data.ack](...data.args);delete this.acknowledgments[data.ack]}}else if(event==='ready'){this.events.connection_ready(this.successfulAttempts>1)}else if(event in this.events){this.events[event](data)}}})}
